...
def _sq_hinge_train(model, data, lr):
    """Train one client using squared hinge loss with SGD."""
...
# ──────────────────────────────────────────────────────────────────────
# FedAvg with Squared Hinge Loss
# ──────────────────────────────────────────────────────────────────────

def client_update(model, data, lr):
    global_model = model.copy()

    model = _sq_hinge_train(model, data, lr)
...
def main_fedavg_sq_hinge(global_parameters, client_schedule, train_data_list, test_data, lr):
    ...
    for round in range(CR):
        ...
        print(f"[FedAvg-SqHinge] Round {round + 1}/{CR} | Acc: {global_acc:.2f}")
