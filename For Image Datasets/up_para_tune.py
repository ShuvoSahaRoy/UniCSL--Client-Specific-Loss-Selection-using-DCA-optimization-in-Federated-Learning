...
from config import *
from Base.utils.utils import setup_logger, set_seed, scenario_suffix, LinearSVM_OVO
...
ALGO_RUNNERS = {
    'fedavg':        lambda gp, tr, te, cs, lr: main_fedavg(gp, tr, te, cs, lr),
    'fedprox':       lambda gp, tr, te, cs, lr: main_fedprox(gp, tr, te, cs, lr),
    'scaffold':      lambda gp, tr, te, cs, lr: main_scaffold(gp, tr, te, cs, lr),
    'feddyn':        lambda gp, tr, te, cs, lr: main_feddyn(gp, tr, te, cs, lr),
    'fedopt':        lambda gp, tr, te, cs, lr: fedopt_main(gp, tr, te, cs, lr),
}
...
for dataset in all_datasets:
    ...
    # (copy.deepcopy(global_parameters), copy.deepcopy(train_data_list), copy.deepcopy(global_test_data), client_schedule, lr)
...
# ── Write to Excel ─────────────────────────────────────────────────────────────
def make_excel(results, lr_list, output_path=f"lr_rate_noniid_{non_iid}_noise_{corrupt_data}.xlsx"):
    wb = Workbook()
...
make_excel(results, LR_LIST, output_path=f"results/lr_rate_noniid_{non_iid}_noise_{corrupt_data}.xlsx")
